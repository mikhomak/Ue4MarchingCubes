    ...
    for (int32 y = 0; y < YScale; y++)
    {
        for (int32 x = 0; x < XScale; x++)
        {
            for (int32 z = 0; z < ZScale; z++)
            {
                ...
                const int Index = GetTriangulationIndexForCube(x, y, z, Points);
                ...
                if (edgeTable[Index] != 0)
                {
                    for (int32 i = 0; triangulation[Index][i] != -1; i += 3)
                    {
                        ...
                        const int A0 = cornerIndexAFromEdge[triangulation[Index][i]];
                        const int B0 = cornerIndexBFromEdge[triangulation[Index][i]];

                        const int A1 = cornerIndexAFromEdge[triangulation[Index][i + 1]];
                        const int B1 = cornerIndexBFromEdge[triangulation[Index][i + 1]];

                        const int A2 = cornerIndexAFromEdge[triangulation[Index][i + 2]];
                        const int B2 = cornerIndexBFromEdge[triangulation[Index][i + 2]];
                        ...
                        /* We do it 3 times cuz it should be correlate wth indexes of Vertices array */
                        const FVector Normal = FVector::CrossProduct(FirstVertice - ThirdVertice,
                                                               SecondVertice - ThirdVertice).GetSafeNormal();
                        Normals.Add(Normal);
                        Normals.Add(Normal);
                        Normals.Add(Normal);
                        ...
                        /* Basic tangets */
                        /* We do it 3 times cuz it should be correlate wth indexes of Vertices array */
                        const FVector Tanget = (FirstVertice - SecondVertice).GetSafeNormal2D();
                        Tangents.Add(Tanget);
                        Tangents.Add(Tanget);
                        Tangents.Add(Tanget);
                    }
